...
function listen(){
	...
	socket.on('notificacionStartGame', procesaNotificacionStartGame);
...
function procesaNotificacionStartGame(data){
	showNotificationStartGame(data);
}
...
function procesaTimerDecrement(data){
	...
	if(data[1] == jugador._nick){
		...
		e_cronometro.css('color' , '#FB0B07');
	}else{
		e_cronometro.css('color' , '#db6b19');
	}
}
